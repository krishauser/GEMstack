...
class ControlSpace:
    """A generic state space with states x and control variable u.  The primary
    function of this class is to define the transformation
...
class Dynamics:
    """A generic differential equation relating state x to control variable u.
    The derivative() method gives diffential constraints x'=f(x,u).

    Convert this into a ControlSpace by using IntegratorControlSpace.
    """
    def stateDimension(self):
        raise NotImplementedError()
    def controlDimension(self):
...
class IntegratorControlSpace (ControlSpace):
    ...
    """
    def __init__(self,dynamics : Dynamics, T : float = 1.0, dt : float=0.01):
