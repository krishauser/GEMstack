...
CURVE_TO_STYLE = {
    RoadgraphCurveEnum.LANE_BOUNDARY: {
        "color": (1, 1, 1, 1),
        "width": 2,
        "pointSize": 0,
    },
    ...
    RoadgraphCurveEnum.CROSSING_BOUNDARY: {
        "color": (1, 1, 1, 1),
        "width": 1,
        "pointSize": 0,
    },
    RoadgraphCurveEnum.PARKING_SPOT_BOUNDARY: {
        "color": (1, 1, 1, 1),
        "width": 1,
        "pointSize": 0,
    },
...
SURFACE_TO_STYLE = {
    ...
    RoadgraphSurfaceEnum.DIRT: {
        "color": (160 / 255.0, 82 / 255.0, 45 / 255.0, 0.2),
        "pointSize": 0,
    },
    RoadgraphSurfaceEnum.GRASS: {
        "color": (50 / 255.0, 255 / 255.0, 50 / 255.0, 0.2),
        "pointSize": 0,
    },
...
REGION_TO_STYLE = {
    RoadgraphRegionEnum.INTERSECTION: {
        "color": (0, 1, 0, 0.5),
        "width": 1,
        "pointSize": 0,
    },
    RoadgraphRegionEnum.PARKING_LOT: {
        "color": (0, 0, 1, 0.5),
        "width": 1,
        "pointSize": 0,
    },
    RoadgraphRegionEnum.CLOSED_COURSE: {
        "color": (1, 0, 0, 0.5),
        "width": 1,
        "pointSize": 0,
    },
...
def plot_object(
    ...
    if outline and obj.outline:
        ...
        if height > 0:
            ...
            outline.append(outline[0])
            vis.add(
                name + "_outline2", outline, width=1, color=core_color, hide_label=True
            )
...
def plot_vehicle(vehicle: VehicleState, vehicle_model=None, axis_len=1.0):
    ...
    plot_object(
        "vehicle", vehicle.to_object(), type=None, axis_len=0, solid=False, label=False
    )
    ...
    vis.add(
        "vehicle_velocity",
        [[front_pt[0], front_pt[1], h], [front_pt[0] + v[0], front_pt[1] + v[1], h]],
        width=2,
        color=(0, 1, 0, 1),
        pointSize=0,
    )
    ...
    if vehicle_model:
        ...
        vis.add(
            "left_wheel",
            [
                vectorops.sub(left_wheel_origin, wheel_offset),
                vectorops.add(left_wheel_origin, wheel_offset),
            ],
            color=(0, 0, 0, 1),
            width=2,
        )
        vis.add(
            "right_wheel",
            [
                vectorops.sub(right_wheel_origin, wheel_offset),
    ...
    # plot gear
    if vehicle.gear in [
        VehicleGearEnum.NEUTRAL,
        VehicleGearEnum.REVERSE,
        VehicleGearEnum.PARK,
    ]:
    ...
    if vehicle.left_turn_indicator:
        lp = vehicle.pose.apply(
            [xbounds[0], ybounds[0] + light_size, turn_indicator_height]
        )
        vis.add(
            "left_turn_indicator", list(lp), size=light_point_size, color=light_color
        )
    if vehicle.right_turn_indicator:
        lp = vehicle.pose.apply(
            [xbounds[0], ybounds[1] - light_size, turn_indicator_height]
        )
        vis.add(
            "right_turn_indicator", list(lp), size=light_point_size, color=light_color
        )
    if vehicle.headlights_on:
        lp = vehicle.pose.apply(
            [xbounds[1], ybounds[0] + light_size * 2, headlight_height]
        )
        vis.add("left_headlight", list(lp), size=light_point_size, color=light_color)
        lp = vehicle.pose.apply(
            [xbounds[1], ybounds[1] - light_size * 2, headlight_height]
        )
        vis.add("right_headlight", list(lp), size=light_point_size, color=light_color)
    if vehicle_model is not None:
        if vehicle.brake_pedal_position > 0.1:
            vehicle_model.link("rear_right_stop_light_link").appearance().setColor(
                1, 0, 0, 1
            )
            vehicle_model.link("rear_left_stop_light_link").appearance().setColor(
                1, 0, 0, 1
            )
        else:
            vehicle_model.link("rear_right_stop_light_link").appearance().setColor(
...
def plot_path(name: str, path: Path, color=(0, 0, 0), width=1):
    if len(path.points) > MAX_POINTS_IN_CURVE:  # downsample due to OpenGL error?
        vis.add(
            name,
            [list(p) for p in path.points[:: len(path.points) // MAX_POINTS_IN_CURVE]],
            color=color,
            width=width,
        )
    else:
...
def plot(
    ...
    plot_scene(
        state,
        ground_truth_vehicle=ground_truth_vehicle,
