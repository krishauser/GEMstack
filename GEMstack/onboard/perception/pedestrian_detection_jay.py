...
class PedestrianDetector(Component):
    ...
    def box_to_agent(self, box, point_cloud_image, point_cloud_image_world): # TODO
        ...
        h1 = 1.7
        x1 = np.min(person_lidar_points[:, 0])
        y1 = np.min(person_lidar_points[:, 1])
        z1 = np.min(person_lidar_points[:, 2])
        d1 = MAX_HUMAN_FATNESS_IN_M
        w1 = MAX_HUMAN_FATNESS_IN_M
        
        
        pose = ObjectPose(t=0,x=x1,y=y1,z=z1,yaw=0,pitch=0,roll=0,frame=ObjectFrameEnum.CURRENT)
        dims = [d1, w1, h1]
    ...
    def detect_agents(self): # TODO
        ...
        pc = np.concatenate((self.point_cloud[:,:3],np.ones((self.point_cloud.shape[0],1))),axis=1)
        pc = (np.dot(self.T_lidar_to_zed, pc.T).T)[:,:3] 

        pc_img, image_indices = project_point_cloud(pc, p_matrix, xrange, yrange)
        pc_img_world = pc[image_indices] 
        ones = np.ones((pc_img_world.shape[0], 1))
        pc_img_world = np.hstack((pc_img_world, ones)) 
        pc_img_world = (np.dot(self.T_zed, pc_img_world.T).T)[:,:3]


        detected_agents = []
        for i,b in enumerate(self.last_person_boxes):
            agent = self.box_to_agent(b, pc_img, pc_img_world)
            detected_agents.append(agent)

        self._point_cloud_image = pc_img
        self._point_cloud_image_world = pc_img_world
    ...
    def track_agents(self, vehicle : VehicleState, detected_agents : List[AgentState]):
        ...
        for agent in detected_agents:
            ...
            for s in self.last_agent_states:
                ...
                last_pose = past_agent
                curr_pose = copy.deepcopy(agent)
                curr_pose.pose.x += vehicle.v * dt

                last_polygon = past_agent.polygon_parent()
                curr_polygon = agent.polygon_parent()
                if collisions.polygon_intersects_polygon_2d(last_polygon, curr_polygon):
                    results[name] = agent
                    v = ((curr_pose.pose.x - last_pose.pose.x) * dt,
                                (curr_pose.pose.y - last_pose.pose.y) * dt,
                                (curr_pose.pose.z - last_pose.pose.z) * dt)
                    agent.velocity = v
                    agent_new = False           
