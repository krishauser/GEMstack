...
class ImageProcessorNode:
    ...
    def process_image(self, cv_image):
        ...
        if bbox_info:
            self.calculate_goal_pose(cv_image, bbox_info)
    ...
    def calculate_goal_pose(self, cv_image, bbox_info):
        ...
        if self.point_cloud:
            ...
            if 0 <= line_mid[0] < coord_3d_map.shape[1] and 0 <= line_mid[1] < coord_3d_map.shape[0]:
                ...
                self.goal_poses.append([red_line_mid_3d[0], red_line_mid_3d[1], angle])

    def save_poses_to_csv(self):
        df = pd.DataFrame(self.goal_poses, columns=['x', 'y', 'yaw'])
        df.to_csv('goal_poses.csv', index=False)
...
if __name__ == '__main__':
    ...
    try:
        ...
        processor_node.save_poses_to_csv()
        print("Shutting down")
