...
class PedestrianDetector2D(Component):
    ...
    def project_points_to_image(self, points_cam: np.ndarray, K: np.ndarray) -> np.ndarray:
        ...
        u = fx * (pts[:, 0] / pts[:, 2]) + cx
        v = fy * (pts[:, 1] / pts[:, 2]) + cy
        # Stack the computed u, v with the original 3D points
        return np.hstack((u.reshape(-1, 1), v.reshape(-1, 1), pts))



    def process_detection(self, box, projected_points, current_time):
        """
        Process a single YOLO detection box to compute the corresponding 3D agent state.

        Args:
            box (np.ndarray): The detection box [center_x, center_y, width, height].
            projected_points (np.ndarray): Array of projected LiDAR points [u, v, X, Y, Z].
            current_time (float): Current time stamp.

        Returns:
            tuple: (new_pose, dims) if successful; otherwise, None.
        """
        cx, cy, w, h = box
        left = int(cx - w / 2)
        right = int(cx + w / 2)
        ...
        # Extract 3D points in the camera frame.
        points_cam_3d = subset_2d[:, 2:5]
        ones = np.ones((points_cam_3d.shape[0], 1), dtype=np.float32)
        points_cam_hom = np.hstack((points_cam_3d, ones))
        # Use the precomputed camera-to-LiDAR transformation.
        points_lidar_hom = (self.T_c2l @ points_cam_hom.T).T
        points_lidar_cluster = points_lidar_hom[:, :3]

        # Remove ground points and refine the cluster.
        ...
        if refined_cluster is None or refined_cluster.shape[0] == 0:
            return None
        intersection = np.mean(refined_cluster, axis=0)
        ...
        # Transform center from LiDAR to vehicle frame.
        ...
        # Create a new pose for the detection.
        new_pose = ObjectPose(
            t=current_time,
            x=center_vehicle[0],
            y=center_vehicle[1],
    ...
    def update(self, vehicle: VehicleState) -> Dict[str, AgentState]:
        ...
        # Run YOLO inference on the current image (detecting only persons, e.g., class 0).
        ...
        agents = {}
        # Downsample LiDAR points for efficiency.
        lidar_pc_down = self.downsample_points(self.latest_lidar, voxel_size=0.1)
        ...
        for box in boxes:
            detection = self.process_detection(box, projected_points, current_time)
            if detection is None:
                continue
            new_pose, dims = detection

            # Match the new detection with an existing agent.
            ...
            if existing_id is not None:
                ...
                vx_smooth, vy_smooth, vz_smooth = exponential_smooth_velocity(
                    (vx_raw, vy_raw, vz_raw), old_smooth_vel, alpha=alpha
                )
