...
class PedestrianDetector2D(Component):
    ...
    def __init__(self,vehicle_interface : GEMInterface):
        ...
        yolo_path = settings.get('pedestrian_detection.model')
    ...
    def image_callback(self, image : cv2.Mat):
        detection_result = self.detector(image)[0]
        boxes = detection_result.boxes
        people_bboxes = []
        for i, class_idx in enumerate(boxes.cls):
            if class_idx == PERSON_INDEX:
                people_bboxes.append(boxes.xywh[i].tolist())
        self.last_person_boxes = people_bboxes
