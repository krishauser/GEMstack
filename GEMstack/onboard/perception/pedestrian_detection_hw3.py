...
class PedestrianDetector(Component):
    ...
    def box_to_agent(self, box, point_cloud_image, point_cloud_image_world):
        ...
        """
        x,y,w,h = box
        inds = np.logical_and(np.logical_and(point_cloud_image[:,0] >= x-w/2,point_cloud_image[:,0] <=x+w/2),
                            np.logical_and(point_cloud_image[:,1] >= y-h/2,point_cloud_image[:,1] <= y+h/2))
        point_bb = point_cloud_image_world[inds]
        point_bb_avg = np.mean(point_bb,axis=0)
        pose = ObjectPose(t=0,x=point_bb_avg[0],y=point_bb_avg[1],z=point_bb_avg[2],yaw=0,pitch=0,roll=0,frame=ObjectFrameEnum.CURRENT)
        min_point = np.min(point_bb,axis=0)
        max_point = np.max(point_bb,axis=0)
        dims = np.abs(max_point-min_point)
        #dims = [w,h,1.7]
    ...
    def detect_agents(self):
        ...
        if len(boxes)!=0:
            for box in boxes:
                if box.cls == 0:
                    if int(box.id.item()) in self.previous_detection_box:
                        self.pair[box.xywh[0]]=self.previous_detection_box[int(box.id.item())]
        ...
        if len(boxes)!=0:
            for box in boxes:
                if box.cls == 0:
                #{new:old}
                    self.previous_detection_box[int(box.id.item())]=box.xywh[0]
    ...
    def track_agents(self, vehicle : VehicleState, detected_agents : List[AgentState]):
        ...
        if self.previous_detections=={}:
            ...
            self.previous_detections=results
        else:
            self.previous_detections=[value for value in self.previous_detections.values()]
