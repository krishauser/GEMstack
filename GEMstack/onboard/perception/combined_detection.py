...
class CombinedDetector3D(Component):
    ...
    def initialize(self):
        ...
        model_path = 'GEMstack/knowledge/detection/epoch_160.pth'
        if torch.cuda.is_available():
            checkpoint = torch.load(model_path)
            self.pointpillars.load_state_dict(checkpoint['model_state_dict'])
            self.pointpillars.to('cuda')
        else:
            checkpoint = torch.load(model_path, map_location=torch.device('cpu'))
            self.pointpillars.load_state_dict(checkpoint['model_state_dict'])
        self.pointpillars.eval()  # Set to evaluation mode
    ...
    def update(self, vehicle: VehicleState) -> Dict[str, AgentState]:
        ...
        # POINTPILLARS TIES IN HERE
        pointpillars_detections = []
        # Get the LiDAR points (X, Y, Z) from the projected points
        lidar_points = projected_pts[:, 2:5]  # Lidar points are in the Lidar coordinate frame
        ...
        with torch.no_grad():
            ...
            if results and len(results) > 0 and 'lidar_bboxes' in results[0]:
                ...
                for i, (bbox, label, score) in enumerate(zip(pp_bboxes, pp_labels, pp_scores)):
                    ...
                    veh_yaw, veh_pitch, veh_roll = R.from_matrix(R_vehicle).as_euler('zyx', degrees=False)
                    
                    # Transform to appropriate frame (START or CURRENT)
                    if self.use_start_frame:
                        ...
                        xp, yp, zp = (T_vehicle_to_start @ np.append(center_vehicle, 1))[:3]
                        out_frame = ObjectFrameEnum.START
                    else:
                        xp, yp, zp = center_vehicle
                        out_frame = ObjectFrameEnum.CURRENT
                    
                    # Determine agent activity based on label
                    # Map PointPillars class indices to your activities
                    if label == 0:  # Adjust mapping as needed
                        activity = AgentActivityEnum.STANDING
                    elif label == 1:
                        activity = AgentActivityEnum.MOVING
                    else:
                        activity = AgentActivityEnum.UNKNOWN
                    
                    # Create pose and record detection
                    pp_pose = ObjectPose(
                        t=current_time,
                        x=xp, y=yp, z=zp,
                        yaw=veh_yaw, pitch=veh_pitch, roll=veh_roll,
                        frame=out_frame
                    )
        ...
        for i, box_info in enumerate(combined_boxes):
            ...
            # Cast rays from the camera through the 2D bounding box into 3D space and capture all LiDAR 
            # points that fall along those rays to get the 3D representation of the detected object:
            mask = (projected_pts[:, 0] >= left) & (projected_pts[:, 0] <= right) & \
                   (projected_pts[:, 1] >= top) & (projected_pts[:, 1] <= bottom)
            roi_pts = projected_pts[mask]
            
            # Ignore regions of interest that don't have many points
            if roi_pts.shape[0] < 5:
                continue

            # Filter the points by distance and depth to remove outliers:
            ...
            # If enabled, use a cylindrical ROI around the mean point position,
            # filter ground points, and depth points:
            if self.use_cyl_roi:
            ...
            # Create an Open3D point cloud with the filtered points and calculate an oriented
            # bounding box to get the object's precise 3D position, dimensions, center, and 
            # orientation
            ...
            # Transform the object's center position from LiDAR to vehicle coordinates
            ...
            # Transforms the object's rotation matrix and converts it to yaw/pitch/roll
            R_vehicle = self.T_l2v[:3, :3] @ R_lidar
            yaw, pitch, roll = R.from_matrix(R_vehicle).as_euler('zyx', degrees=False)
            refined_center = refined_center_vehicle

            if self.use_start_frame:
                # Transform coordinates to the starting vehicle frame
                if self.start_pose_abs is None:
                    self.start_pose_abs = vehicle.pose
                vehicle_start_pose = vehicle.pose.to_frame(
                    ObjectFrameEnum.START,
            ...
            if self.enable_tracking:
                ...
                if existing_id is not None:
                    ...
                    agent_id = f"Cone{self.ped_counter}"
                    self.ped_counter += 1
...
class FakCombinedDetector(Component):
