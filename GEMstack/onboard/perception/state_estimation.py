...
class GNSSStateEstimator(Component):
    ...
    def update(self) -> VehicleState:
        ...
        # filtering speed
        # current_t = self.vehicle_interface.time()
        # if self.prev_t is not None and self.gnss_speed > 100.0:
        #     # GNSS problem, try correct with xy coordinate
        #     print("gnss problem")
        #     print("offset: ", ((raw.pose.x - self.prev_x) ** 2 + (raw.pose.y - self.prev_y)**2)**0.5)
        #     raw.v = ((raw.pose.x - self.prev_x) ** 2 + (raw.pose.y - self.prev_y)**2)**0.5 / (current_t - self.prev_t)
        # else:
        #     raw.v = self.gnss_speed
        # print(raw.v)
        # self.prev_x = raw.pose.x
        # self.prev_y = raw.pose.y
        ...
        if raw.gear == -1:
            raw.v *= -1
        print("velocity from gnss: ", raw.v)
