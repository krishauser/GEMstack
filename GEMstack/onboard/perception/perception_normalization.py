...
class StandardPerceptionNormalizer(Component):
    ...
    def update(self,state : AllState):
        ...
        state.roadgraph = state.roadgraph.to_frame(ObjectFrameEnum.CURRENT, current_pose=state.vehicle, start_pose_abs=state.start_vehicle_pose)

        for k,a in state.agents.items():
            a.pose = a.pose.to_frame(ObjectFrameEnum.CURRENT, current_pose=state.vehicle, start_pose_abs=state.start_vehicle_pose)
        
        for k,o in state.obstacles.items():
            o.pose = o.pose.to_frame(ObjectFrameEnum.CURRENT, current_pose=state.vehicle, start_pose_abs=state.start_vehicle_pose)
            ...
            for pred in a.predictions:
                if pred.path is not None:
                    for i,p in enumerate(pred.path):
                        pred.path[i] = p.to_frame(ObjectFrameEnum.CURRENT, current_pose=state.vehicle, start_pose_abs=state.start_vehicle_pose)
