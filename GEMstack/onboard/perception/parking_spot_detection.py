...
class ParkingSpotDetector(Component):
    ...
    def parking_spot_detection(self):
        if self.front_image is None or self.point_cloud is None:
            return  # Just return without doing anything if data is not ready

        if self.euclidean is not None and self.euclidean > self.dist_thresh:
            return  # Do nothing if the object is out of range

        bbox_info = self.detect_empty(self.front_image)
        if not bbox_info:
            return  # Do nothing if no empty parking spot is detected

        canvas = self.front_image.copy()
        goal_pose = self.get_parking_spot(canvas, bbox_info)
        if not goal_pose:
            return  # Do nothing if goal pose calculation fails
    ...
    def get_parking_spot(self, img, bbox):
        pixel_coords, img = self.isolate_and_draw_lines(img, bbox)
        angle = pixel_coords[1]
        coords_3d = self.get_goal_pose(pixel_coords)
        x,y,z = coords_3d
    ...
    def update(self):
        previous_spot = self.parking_spot  # Store the previous state
        self.parking_spot_detection()  # Attempt to detect and update parking spot
        if self.parking_spot is None:
            self.parking_spot = previous_spot  # Restore previous state if no update was made
