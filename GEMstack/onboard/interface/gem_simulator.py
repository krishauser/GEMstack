...
class SignalSimulation:
    def __init__(self, config):
        ...
        self.idx = list(SIGNAL_STATE_TO_ENUM.keys()).index(self.state) # index of start state in dict
    ...
    def to_sign_state(self) -> Sign:
        ...
        duration = self.duration[list(SIGNAL_STATE_TO_ENUM.keys()).index(self.state)]
        return Sign(pose=pose, entities=[], dimensions=[0.25,0.25,4], outline=[],
...
class GEMDoubleIntegratorSimulationInterface(GEMInterface):
    ...
    def __init__(self, scene : str = None):
        ...
        self.signal_emulator_settings = settings.get('simulator.signal_emulator',{})
        ...
        self.signal_dt = self.signal_emulator_settings.get('dt',0.1)
        ...
        self.signal_detector_callback = None
    ...
    def subscribe_sensor(self, name, callback, type = None):
        if name == 'gnss':
            ...
            self.signal_detector_callback = callback
        else:
    ...
    def simulate(self,lock : Lock, data : dict):
        ...
        while looper and not data['stop']:
            with lock:
                ...
                if self.signal_detector_callback is not None and self.simulator.simulation_time - self.last_signal_time > self.signal_dt:
                    for k,s in self.simulator.signals.items():
                        self.signal_detector_callback(k,s.to_sign_state())
                    self.last_signal_time = self.simulator.simulation_time
