        ...
        while progress < progress_end:
            ...
            if len(all_pedestrians) > 0:
                ...
                for ped in all_pedestrians:
                    ped_dist_list.append(np.hypot(vehicle.pose.x - ped.pose.x, vehicle.pose.y - ped.pose.y))
                closest_ped = all_pedestrians[np.argmin(ped_dist_list)]
                
                V_x = vehicle.pose.x
                V_y = vehicle.pose.y
                V_l = 4.0
                V_w = 1.6
                P_x = closest_ped.pose.x
                P_y = closest_ped.pose.y
                P_l = closest_ped.dimensions[0]
                P_w = closest_ped.dimensions[1]
                margin_x = 3.0
                margin_y = 1.0
                ld_braking = (vehicle.v)**2 / (2 * self.deceleration) # self.desired_speed
                
                delta_x = abs(P_x - V_x) - 0.5*P_l - 0.5*V_l
                delta_y = abs(P_y - V_y) - 0.5*P_w - 0.5*V_w
                if (delta_x < margin_x + ld_braking) and (delta_y < margin_y):
