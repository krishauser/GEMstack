...
def waypoint_generate_relaxed(vehicle_state, cones, cone_idx, prev_final_state_guess):
    """
    Generate waypoints based on the scenario.
    Args:
        vehicle_state: The state of the vehicle.
    ...
    scenario, cone_position = scenario_check(vehicle_state, [cones[cone_idx]])
    car_position = np.array(vehicle_state['position'])
    if prev_final_state_guess['psi'] is not None:
          car_heading = prev_final_state_guess['psi']
    else:
        car_heading = vehicle_state['heading'] # in radians
    ...
    if scenario == 'standing':
        ...
        for i in range(num_arc_points):
            ...
            # Derivative of the arc gives tangent vector (for heading)
            # d/dθ of position function -> tangent vector direction
            tangent_vec = (
                u_turn_radius * (
                    perpendicular_vector * np.sin(angle) +
                    heading_vector * np.cos(angle)
                )
            )
            
            flex_wps_list.append(wp)
            wp_heading_list.append(tangent_vec)
        ...
        flex_wp1 = cone + offset * perpendicular_vector
        flex_wps_list = [flex_wp1]
        wp_heading_list = [heading_vector]
    ...
    return scenario, flex_wps_list, fixed_wp, target_heading, fixed_wp_list, wp_heading_list
...
def trajectory_generation_dynamics(init_state, final_state, N=30, Lr=1.5,
    ...
    def waypoint_penalty(p):
        ...
        for wp_idx, waypoint in enumerate(waypoints):
            ...
            if waypoint_headings is not None and wp_idx < len(waypoint_headings):
                # Vehicle heading vector from psi
                vehicle_heading_vector = np.array([np.cos(psi_[traj_idx]), np.sin(psi_[traj_idx])])
                waypoint_heading_vector = np.array(waypoint_headings[wp_idx])

                # Normalize to avoid scale issues
                vehicle_heading_vector /= np.linalg.norm(vehicle_heading_vector)
                waypoint_heading_vector /= np.linalg.norm(waypoint_heading_vector)

                # Dot product → 1 if aligned, -1 if opposite
                dot = np.dot(vehicle_heading_vector, waypoint_heading_vector)
                if dot < alignment_threshold:
                    continue
                
                # alignment_error = 1 - dot  # 0 when aligned, max = 2 when opposite

                # penalty += waypoint_penalty_weight * alignment_error**2
...
def plan_full_slalom_trajectory(vehicle_state, cones):
    ...
    waypoint_all = []
    fixed_wp_all = []
    waypoint_heading_all = []

    final_state = {
            'x': None, 'y': None, 'psi': None, 'c': None
        }

    for cone_idx, cone in enumerate(cones):
        # scenario, flex_wps, fixed_wp, target_heading = waypoint_generate(vehicle_state, cones, cone_idx)
        scenario, flex_wps, fixed_wp, target_heading, fixed_wp_list, waypoint_heading_list = waypoint_generate_relaxed(vehicle_state, cones, cone_idx, prev_final_state_guess=final_state)
        if not flex_wps or fixed_wp is None:
            continue
        # flex_wp = flex_wps[0]
        waypoint_all.extend(flex_wps)
        fixed_wp_all.extend(fixed_wp_list)
        waypoint_heading_all.extend(waypoint_heading_list)
    ...
    for i, wp in enumerate(waypoint_all):
        ...
        heading = waypoint_heading_all[i]
        # print(heading)
        dx = 1.5 * heading[0]
        dy = 1.5 * heading[1]
        plt.arrow(wp[0], wp[1], dx, dy, head_width=0.4, head_length=0.6, fc='k', ec='k')
