    ...
    def update(self, vehicle: VehicleState, agents: Dict[str, AgentState], mission_plan: MissionPlan, obstacles: Obstacle) -> Route:
        ...
        for n, o in obstacles.items():
            ...
            if o.type == ObstacleMaterialEnum.TRAFFIC_CONE:
                print("CONE detected")
                obstacle_global_pose = o.pose.to_frame(ObjectFrameEnum.GLOBAL, current_pose = vehicle.pose, start_pose_abs=mission_plan.start_vehicle_pose)
        ...
        if self.route == None:
            self.rects = self.occupancy_grid.draw_obstacle_cones(obstacles_global_poses)
        ...
        if mission_plan.planner_type.value == PlannerEnum.PARKING.value:
            ...
            for i, (rect_pt1_x, rect_pt1_y, rect_pt2_x, rect_pt2_y) in enumerate(self.rects):
            ...
            # start_w = [start_x, start_y,  start_yaw]   for flipping direction
            # goal_w = [goal_x, goal_y, goal_yaw + math.pi]

            start_w = [start_x, start_y,  start_yaw + math.pi]
            goal_w = [goal_x, goal_y, goal_yaw + math.pi]
...
#             if o.type == ObstacleMaterialEnum.TRAFFIC_CONE:
#                 print("CONE detected")
#                 obstacle_global_pose = o.pose.to_frame(ObjectFrameEnum.ABSOLUTE_CARTESIAN, start_pose_abs=mission_plan.start_vehicle_pose)
#                 print("obstacle_global_pose", obstacle_global_pose)
#                 obstacles_global_poses.append((obstacle_global_pose.y, obstacle_global_pose.x))
...
#             ## Step 1: Convert vehicle pose to global frame
#             vehicle_global_pose = vehicle.pose.to_frame(
#                 ObjectFrameEnum.ABSOLUTE_CARTESIAN, start_pose_abs=mission_plan.start_vehicle_pose
#             )
