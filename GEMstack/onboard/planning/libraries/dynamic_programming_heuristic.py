...
#     return rx, ry
def calc_final_path(self, goal_node, closed_set, resolution):
    # generate final course
    rx, ry = [self.calc_grid_position(goal_node.x, self.min_x, resolution)], [
        self.calc_grid_position(goal_node.y, self.min_y, resolution)]
    parent_index = goal_node.parent_index
    while parent_index != -1:
        n = closed_set[parent_index]
        rx.append(self.calc_grid_position(n.x, self.min_x, resolution))
        ry.append(self.calc_grid_position(n.y, self.min_y, resolution))
        parent_index = n.parent_index

    return rx, ry
def calc_grid_position(self, index, min_position, resolution):
    """
    calc grid position
