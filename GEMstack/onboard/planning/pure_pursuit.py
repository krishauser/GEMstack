...
class PurePursuit(object):
    ...
    def __init__(self, lookahead=None, lookahead_scale=None, crosstrack_gain=None, desired_speed=None):
        ...
        self.look_ahead_scale = lookahead_scale if lookahead_scale is not None else settings.get(
            'control.pure_pursuit.lookahead_scale', 3.0)
        self.crosstrack_gain = crosstrack_gain if crosstrack_gain is not None else settings.get(
            'control.pure_pursuit.crosstrack_gain', 0.41)
        ...
        self.pid_speed = PID(settings.get('control.longitudinal_control.pid_p', 0.5),
                             ...
                             settings.get('control.longitudinal_control.pid_i', 0.1), windup_limit=20)
    ...
    def compute(self, state: VehicleState, component: Component = None):
        ...
        closest_dist, closest_parameter = self.path.closest_point_local((curr_x, curr_y),
                                                                        [self.current_path_parameter - 5.0,
                                                                         self.current_path_parameter + 5.0])
        ...
        print("Desired point", (desired_x, desired_y), " with lookahead distance",
              self.look_ahead + self.look_ahead_scale * speed)
...
class PurePursuitTrajectoryTracker(Component):
    ...
    def update(self, vehicle: VehicleState, trajectory: Trajectory):
        ...
        steering_angle = np.clip(front2steer(wheel_angle), self.pure_pursuit.steering_angle_range[0],
