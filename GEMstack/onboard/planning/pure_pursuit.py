...
class PurePursuit(object):
    ...
    def __init__(
        self,
        lookahead=None,
        lookahead_scale=None,
        crosstrack_gain=None,
        desired_speed=None,
    ):
        self.look_ahead = (
            lookahead
            if lookahead is not None
            else settings.get("control.pure_pursuit.lookahead", 4.0)
        )
        self.look_ahead_scale = (
            lookahead_scale
            if lookahead_scale is not None
            else settings.get("control.pure_pursuit.lookahead_scale", 3.0)
        )
        self.crosstrack_gain = (
            crosstrack_gain
            if crosstrack_gain is not None
            else settings.get("control.pure_pursuit.crosstrack_gain", 0.41)
        )
        self.front_wheel_angle_scale = 3.0
        self.wheelbase = settings.get("vehicle.geometry.wheelbase")
        self.wheel_angle_range = [
            settings.get("vehicle.geometry.min_wheel_angle"),
            settings.get("vehicle.geometry.max_wheel_angle"),
        ]
        self.steering_angle_range = [
            settings.get("vehicle.geometry.min_steering_angle"),
        ...
        if desired_speed is not None:
            ...
            self.desired_speed_source = settings.get(
                "control.pure_pursuit.desired_speed", "path"
            )
        self.desired_speed = (
            self.desired_speed_source
        ...
        self.pid_speed = PID(
            settings.get("control.longitudinal_control.pid_p", 0.5),
    ...
    def set_path(self, path: Path):
        ...
        if not isinstance(path, Trajectory):
            ...
            if self.desired_speed_source in ["path", "trajectory"]:
                raise ValueError(
                    "Can't provide an untimed path to PurePursuit and expect it to use the path velocity. Set control.pure_pursuit.desired_speed to a constant."
                )
        else:
            self.path = path.arc_length_parameterize()
    ...
    def compute(self, state: VehicleState, component: Component = None):
        ...
        if self.path is None:
            # car doesn't move if there isn't a path
            return (0, 0)

        if self.path.frame != state.pose.frame:
            print(
                "Transforming path from",
                self.path.frame.name,
                "to",
                state.pose.frame.name,
            )
            self.path = self.path.to_frame(state.pose.frame, current_pose=state.pose)
        if self.trajectory is not None:
            if self.trajectory.frame != state.pose.frame:
                print(
                    "Transforming trajectory from",
                    self.trajectory.frame.name,
                    "to",
                    state.pose.frame.name,
                )
                self.trajectory = self.trajectory.to_frame(
                    state.pose.frame, current_pose=state.pose
                )

        closest_dist, closest_parameter = self.path.closest_point_local(
            (curr_x, curr_y),
            [self.current_path_parameter - 5.0, self.current_path_parameter + 5.0],
        )
        ...
        des_parameter = (
            closest_parameter + self.look_ahead + self.look_ahead_scale * speed
        )
        print(
            "Closest parameter: " + str(closest_parameter),
            "distance to path",
            closest_dist,
        )
        if closest_dist > 0.1:
            print(
                "Closest point",
                self.path.eval(closest_parameter),
                "vs",
                (curr_x, curr_y),
            )
        if des_parameter >= self.path.domain()[1]:
            ...
            desired_x, desired_y = transforms.vector_madd(
                end_pt, end_dir, (des_parameter - self.path.domain()[1])
            )
        else:
        ...
        print(
            ...
            + str(round(np.degrees(steering_angle), 2))
            + " degrees"
        )
        ...
        if self.desired_speed_source in ["path", "trajectory"]:
            ...
            if (
                len(self.trajectory.points) < 2
                or self.current_path_parameter >= self.path.domain()[1]
            ):
                ...
                if self.desired_speed_source == "path":
                    current_trajectory_time = self.trajectory.parameter_to_time(
                ...
                if current_trajectory_time >= self.trajectory.domain()[1]:
                    prev_deriv = self.trajectory.eval_derivative(
                        current_trajectory_time - difference_dt
                    )
                    prev_desired_speed = min(
                        np.linalg.norm(prev_deriv), self.speed_limit
                    )
                    feedforward_accel = (
                        desired_speed - prev_desired_speed
                    ) / difference_dt
                    print(
                        "Desired speed",
                        desired_speed,
                    ...
                    next_deriv = self.trajectory.eval_derivative(
                        current_trajectory_time + difference_dt
                    )
                    next_desired_speed = min(
                        np.linalg.norm(next_deriv), self.speed_limit
                    )
                    feedforward_accel = (
                        next_desired_speed - desired_speed
                    ) / difference_dt
                    print(
                        "Desired speed",
                        desired_speed,
                        "m/s",
                        ", trying to reach desired",
                        next_desired_speed,
                        "m/s",
                    )
                feedforward_accel = np.clip(
                    feedforward_accel, -self.max_decel, self.max_accel
                )
        ...
        output_accel = self.pid_speed.advance(
            e=desired_speed - speed, t=t, feedforward_term=feedforward_accel
        )
        if component is not None:
...
class PurePursuitTrajectoryTracker(Component):
    ...
    def update(self, vehicle: VehicleState, trajectory: Trajectory):
        ...
        steering_angle = np.clip(
            ...
            self.pure_pursuit.steering_angle_range[0],
            self.pure_pursuit.steering_angle_range[1],
        )
