...
class Stanley(object):
    ...
    def compute(self, state: VehicleState, component: Component = None):
        ...
        if self.brake or speed_error < 1:
            print("speed error: "+ str(speed_error))
            self.brake = True
            return (100, speed)
        # Clip acceleration
...
class StanleyTrajectoryTracker(Component):
    ...
    def update(self, vehicle: VehicleState, trajectory: Trajectory):
        ...
        if self.brake or accel == 100.0:
            self.brake = True
            print("brake: " +str(self.brake))
            print("pedal pos:" +str(self.pedal))
            # current speed from fdelta
            if f_delta > 0:
                speed = f_delta
                speed_diff = speed - self.prev_speed
                print("speed: "+str(speed))
                print("prev speed: "+ str(self.prev_speed))
                print("speed diff: "+str(speed_diff))
                if speed_diff < -0.5:
                    self.pedal += 0.1
                    self.prev_speed = speed
                cmd = self.vehicle_interface.simple_command(100, self.pedal, vehicle)
                self.vehicle_interface.send_command(cmd)
            else:
                cmd = self.vehicle_interface.simple_command(100, self.pedal, vehicle)
                self.vehicle_interface.send_command(cmd)
        else:
            self.prev_speed = vehicle.v
