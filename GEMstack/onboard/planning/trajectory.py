...
class Path:
    ...
    def adjust_speed(self, curve, max_speed, k=3.0):
        ...
        print("adjust speed inverse--------", speed_inv)
        print("adjust speed expo--------", speed_exp)
    ...
    def var_speed_arc_length_parameterize(self, reference_speed) -> 'Trajectory':
        ...
        print("------------------------", headings)
        # Assume self.points is a list of [x, y] coordinates
        for i in range(1, len(self.points) - 1):
            ...
            yaw = headings[i-1]
            next_yaw = headings[i]
            # print("p0[0], p0[1]", p0[0], p0[1])
            # print("p1[0], p1[1]",p1[0], p1[1])
            # print("p2[0], p2[1]",p2[0], p2[1])
            yaw_difference = abs(next_yaw - yaw)
            print("yaw difference: ", yaw_difference)
            # these two lines are write by myself
            curve = self.curvature(p0[0], p0[1], p1[0], p1[1], p2[0], p2[1])
            #adjusted_speed = self.adjust_speed(curve, reference_speed, 1)
            curvature_adjusted_speed = self.adjust_speed(curve, reference_speed, k=1)  # 'k' is a factor to tweak
    
            # Define yaw thresholds in radians (e.g., 10 degrees, 20 degrees)
            low_threshold = np.radians(3)  # Smaller changes
            high_threshold = np.radians(5)  # Larger changes
            
            # Define speeds corresponding to the yaw differences
            if yaw_difference < low_threshold:
                adjusted_speed = reference_speed  # No change for small yaw differences
            elif low_threshold <= yaw_difference < high_threshold:
                adjusted_speed = reference_speed * 0.6  # Reduce speed by 25% for moderate changes
            else:
                adjusted_speed = reference_speed * 0.2  # Reduce speed by 50% for large changes
