...
def normalize_computation_graph(components: list) -> List[Dict]:
    ...
    for c in components:
        if isinstance(c, str):
            ...
            assert isinstance(
                v, dict
            ), "Component {} value is not a string or dict".format(c)
            if "inputs" not in v:
                v["inputs"] = []
...
def load_computation_graph():
    ...
    COMPONENTS = normalize_computation_graph(
        settings.get("run.computation_graph.components")
    )
...
def make_class(config_info, component_module, parent_module=None, extra_args=None):
    ...
    try:
        ...
        try:
            ...
            executor_debug_print(
                0,
                "Unable to launch module {} with class {} and args {} kwargs {}",
                component_module,
                class_name,
                args,
                kwargs,
...
def validate_components(
    components: Dict[str, ComponentExecutor], provided: List = None
):
    ...
    for k in COMPONENT_ORDER:
        ...
        for i in inputs:
            if i == "all":
                assert possible_inputs == [
                    "all"
                ], "Component {} inputs are not provided by previous components".format(
                    k
                )
            else:
                assert (
                    provided_all or i in provided
                ), "Component {} input {} is not provided by previous components".format(
                    k, i
                )
                if i not in state:
                    executor_debug_print(
                        "Component {} input {} does not exist in AllState object", k, i
                    )
                if possible_inputs != ["all"]:
                    assert (
                        i in possible_inputs
        ...
        for o in required_outputs:
            ...
            if o == "all":
                ...
                assert (
                    o in outputs
                ), "Component {} doesn't output required output {}".format(k, o)
        for o in outputs:
            if "all" != o:
                provided.add(o)
                if o not in state:
                    executor_debug_print(
                        "Component {} output {} does not exist in AllState object", k, o
                    )
            else:
                provided_all = True
    for k, c in components.items():
        executor_debug_print(
            0, "Component {} uses implementation {}", k, c.c.__class__.__name__
        )
...
class ComponentExecutor:
    ...
    def update(self, t: float, state: AllState):
        if self.next_update_time is None or t >= self.next_update_time:
            ...
            if self.next_update_time < t and self.dt > 0:
                if t1 - t0 > self.dt:
                    executor_debug_print(
                        1,
                        "Component {} is running behind, time {} overran dt {} by {} s",
                        self.c.__class__.__name__,
                        t1 - t0,
                        self.dt,
                        t - self.next_update_time,
                    )
                else:
                    executor_debug_print(
                        1,
                        "Component {} is running behind (pushed back) overran dt {} by {} s",
                        self.c.__class__.__name__,
                        t1 - t0,
                        self.dt,
                        t - self.next_update_time,
        ...
        executor_debug_print(
            3,
            "Component {}",
            "not updating at time {}, next update time is {}",
            self.c.__class__.__name__,
            t,
        ...
        with contextlib.redirect_stdout(f):
            with contextlib.redirect_stderr(g):
                try:
                    ...
                    executor_debug_exception(
                        e, "Exception in component {}: {}", self.c.__class__.__name__, e
                    )
    ...
    def update_now(self, t: float, state: AllState):
        ...
        if res is not None:
            if len(self.output) > 1:
                assert len(res) == len(
                    self.output
                ), "Component {} output {} does not match expected length {}".format(
                    self.c.__class__.__name__, self.output, len(self.output)
                )
...
class ExecutorBase:
    ...
    def __init__(self, vehicle_interface):
        ...
        self.pipelines = (
            dict()
        )  # type: Dict[str,Tuple[Dict[str,ComponentExecutor],Dict[str,ComponentExecutor],Dict[str,ComponentExecutor]]]
    ...
    def make_component(
        self, config_info, component_name, parent_module=None, extra_args=None
    ) -> ComponentExecutor:
        ...
        if identifier in self.all_components:
            ...
            try:
                component = make_class(
                    config_info, component_name, parent_module, extra_args
                )
            except Exception as e:
                executor_debug_exception(
                    e,
                    "Exception raised while trying to make component {} from config info:\n   {}",
                    component_name,
                    config_info,
                )
                raise
            if not isinstance(component, Component):
                raise RuntimeError(
                    "Component {} is not a subclass of Component".format(component_name)
                )
            replacement = self.logging_manager.component_replayer(
                component_name, component
            )
            if replacement is not None:
                executor_debug_print(
                    1,
                    "Replaying component {} from long {} with outputs {}",
                    component_name,
                    replacement.logfn,
                    component.state_outputs(),
                )
            ...
            if isinstance(config_info, dict):
                ...
                executor.print_stderr = executor.print_stdout = config_info.get(
                    "print", True
                )
    ...
    def add_pipeline(
        self,
        name: str,
        perception: Dict[str, ComponentExecutor],
    ...
    def log_vehicle_interface(self, enabled=True):
        ...
        if enabled:
            ...
            raise NotImplementedError(
                "Disabling vehicle interface logging not supported yet"
            )
    ...
    def log_ros_topics(
        self, topics: List[str], rosbag_options: str = ""
    ) -> Optional[str]:
    ...
    def run(self):
        ...
        )  # kludge! should refactor to avoid global variables

        # sanity checking
        if self.current_pipeline not in self.pipelines:
            executor_debug_print(
                0, "Initial pipeline {} not found", self.current_pipeline
            )
            return
        ...
        for c in self.logging_manager.replayed_components.keys():
            ...
            for name, (
                ...
                if (
                    c in perception_components
                    or c in planning_components
                    or c in other_components
                ):
                    found = True
                    break
            if not found:
        ...
        try:
            ...
            if not validated:
                self.event(
                    ...
                    "Could not validate sensors, stopping components and exiting",
                )
                self.set_exit_reason("Sensor validation failed")
        except KeyboardInterrupt:
            self.event(
                "Ctrl+C interrupt during sensor validation",
                "Could not validate sensors, stopping components and exiting",
            )
            self.set_exit_reason("Sensor validation failed")
        ...
        if validated:
            ...
            while True:
                ...
                try:
                    executor_debug_print(
                        1, "Executing pipeline {}", self.current_pipeline
                    )
                    ...
                    if next not in self.pipelines:
                        executor_debug_print(
                            1, "Pipeline {} not found, switching to recovery", next
                        )
                        next = "recovery"
                    ...
                    if not self.validate_sensors(1):
                        self.event(
                            "Sensors in desired pipeline {} are not working, switching to recovery".format(
            ...
            # done with mission
            self.event(
                "Mission execution ended",
                "Done with mission execution, stopping components and exiting",
            )
    ...
    def check_for_hardware_faults(self):
        ...
        if printed_faults:
            if EXECUTION_VERBOSITY >= 1:
                fault_strings = [
                    (f + " (new)" if f in new_faults else f) for f in printed_faults
                ]
    ...
    def validate_sensors(self, numsteps=None):
        ...
        (perception_components, planning_components, other_components) = self.pipelines[
            self.current_pipeline
        ]
        if len(perception_components) == 0:
            return True
        components = list(perception_components.values()) + list(
            self.always_run_components.values()
        )
        ...
        while looper and not sensors_working:
            ...
            always_run_working = all(
                [c.healthy() for c in self.always_run_components.values()]
            )
    ...
    def run_until_switch(self):
        ...
        components = (
            list(perception_components.values())
            + list(planning_components.values())
            + list(other_components.values())
            + list(self.always_run_components.values())
        )
        dt_min = min([c.dt for c in components if c.dt != 0.0])
        looper = TimedLooper(dt_min, name="main executor")
        while looper and not self.done():
            ...
            for name, c in perception_components.items():
                if not c.healthy():
                    if c.essential and self.current_pipeline != "recovery":
                        executor_debug_print(
                            1, "Sensor {} not working, entering recovery mode", name
                        )
                        return "recovery"
                    else:
                        executor_debug_print(
                            1, "Warning, sensor {} not working, ignoring", name
                        )

            next_pipeline = self.update(self.state)
            if next_pipeline is not None and next_pipeline != self.current_pipeline:
                executor_debug_print(
                    0, "update() requests to switch to pipeline {}", next_pipeline
                )
            ...
            for name, c in planning_components.items():
                if not c.healthy():
                    if c.essential and self.current_pipeline != "recovery":
                        executor_debug_print(
                            1, "Planner {} not working, entering recovery mode", name
                        )
                        return "recovery"
                    else:
                        executor_debug_print(
                            1, "Warning, planner {} not working, ignoring", name
                        )
            ...
            for name, c in other_components.items():
                if not c.healthy():
                    if c.essential and self.current_pipeline != "recovery":
                        executor_debug_print(
                            1,
                            "Other component {} not working, entering recovery mode",
                            name,
                        )
                        return "recovery"
                    elif c.api_call:
            ...
            for name, c in self.always_run_components.items():
                if not c.healthy():
                    if c.essential and self.current_pipeline != "recovery":
                        executor_debug_print(
                            1,
                            "Always-run component {} not working, entering recovery mode",
                            name,
                        )
                        return "recovery"
                    else:
                        executor_debug_print(
                            1, "Warning, always-run component {} not working", name
                        )
    ...
    def update_components(
        ...
        for k in order:
            ...
            if updated:
                self.logging_manager.log_component_update(
                    k, state, components[k].output
                )
...
class StandardExecutor(ExecutorBase):
    ...
    def done(self):
        if self.current_pipeline == "recovery":
            if (
                self.vehicle_interface.last_reading is not None
                and abs(self.vehicle_interface.last_reading.speed) < 1e-3
            ):
                executor_debug_print(1, "Vehicle has stopped, exiting execution loop.")
                return True
            if "disengaged" in self.vehicle_interface.hardware_faults():
                executor_debug_print(
                    1, "Vehicle has disengaged, exiting execution loop."
                )
                return True
        return False
