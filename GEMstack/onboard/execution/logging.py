...
class LoggingManager:
    ...
    def set_log_folder(self, folder: str) -> None:
        ...
        self.run_metadata["start_time_human_readable"] = (
    ...
    def replay_components(self, replayed_components: list, replay_folder: str):
        ...
        settings = config.load_config_recursive(
            os.path.join(replay_folder, "settings.yaml")
        )
        try:
    ...
    def component_replayer(
        ...
        if component_name in self.replayed_components:
            ...
            ), "Replayed component {} must have a positive rate".format(component_name)
            return LogReplay(
                outputs, os.path.join(replay_folder, "behavior_log.json"), rate=rate
            )
        return None
    ...
    def dump_log_metadata(self):
        ...
        config.save_config(
            os.path.join(self.log_folder, "meta.yaml"), self.run_metadata
        )
    ...
    def load_log_metadata(self):
        ...
        self.run_metadata = config.load_config_recursive(
            os.path.join(self.log_folder, "meta.yaml")
        )
    ...
    def log_vehicle_behavior(self, vehicle_interface) -> VehicleBehaviorLogger:
        ...
        if self.behavior_log is None:
            self.behavior_log = logging.Logfile(
                os.path.join(self.log_folder, "behavior.json"),
                delta_format=True,
                mode="w",
            )
    ...
    def log_state(
        self, state_attributes: List[str], rate: Optional[float] = None
    ) -> AllStateLogger:
        if not self.log_folder:
            return
    ...
    def log_ros_topics(
        ...
        if topics:
            command = [
                "rosbag",
                "record",
                "--output-name={}".format(os.path.join(self.log_folder, "vehicle.bag")),
            ]
    ...
    def event(self, event_description: str):
        ...
        self.run_metadata["events"].append(
            {
                "time": time.time(),
    ...
    def debug(self, component: str, item: str, value: Any) -> None:
        ...
        if isinstance(value, (list, tuple)):
            ...
            filename = os.path.join(
                folder, item + "_%03d.npz" % len(self.debug_messages[component][item])
            )
    ...
    def pipeline_start_event(self, pipeline_name: str) -> None:
        ...
        self.run_metadata["pipelines"].append(
            {
                "time": time.time(),
                "vehicle_time": self.vehicle_time,
                "name": pipeline_name,
    ...
    def log_component_update(
        self, component: str, state: Any, outputs: List[str]
    ) -> None:
        """Component update"""
    ...
    def log_component_stdout(self, component: str, msg: List[str]) -> None:
        ...
        if self.component_output_loggers[component][0] is None:
            self.component_output_loggers[component][0] = open(
                self.component_stdout_file(component), "w"
            )
        timestr = datetime.datetime.fromtimestamp(self.vehicle_time).strftime(
            "%H:%M:%S.%f"
        )[:-3]
        for l in msg:
    ...
    def log_component_stderr(self, component: str, msg: List[str]) -> None:
        ...
        if self.component_output_loggers[component][1] is None:
            self.component_output_loggers[component][1] = open(
                self.component_stderr_file(component), "w"
            )
        timestr = datetime.datetime.fromtimestamp(self.vehicle_time).strftime(
            "%H:%M:%S.%f"
        )[:-3]
    ...
    def close(self):
        ...
        if self.rosbag_process is not None:
            ...
            print(
                "Log file size in MegaBytes is {}".format(
                    loginfo.st_size / (1024 * 1024)
                )
            )
            print("-------------------------------------------")
            self.rosbag_process = None
...
class LogReplay(Component):
    ...
    def update(self):
        ...
        res, msgs = self.logfile.read(
            duration_from_start=(t - self.start_time) * self.speed_multiplier,
            cumulative=True,
        )
...
class VehicleBehaviorLogger(Component):
    ...
    def update(self, state):
        if state.t != self.vehicle_log_t_last:
            collection = {
                "vehicle_interface_command": self.vehicle_interface.last_command,
