...
def generate_behavior_plots(log_folder, behavior_file, target_frame=3):
        ...
        for line in f:
            try:
                ...
                if 'vehicle' in entry and 'data' in entry['vehicle']:
                    ...
                    if target_frame is None or vehicle_state.get('frame') == target_frame:
                        vehicle_data.append({
                            'time': entry['time'],
                ...
                if 'agents' in entry:
                    for agent_name, agent_info in entry['agents'].items():
                        ...
                        # Check frame filter if specified
                        if target_frame is None or agent_state.get('frame') == target_frame:
                ...
                if 'trajectory' in entry and 'data' in entry['trajectory']:
                    ...
                    traj_frames = entry['trajectory']['data'].get('frames', [None] * len(traj_points))
                    
                    trajectory_data = [
                        {'x': point[0], 'y': point[1], 'time': time, 'frame': frame}
                        for point, time, frame in zip(traj_points, traj_times, traj_frames)
    ...
    if vehicle_data:
        ...
        plt.plot(vehicle_xs, vehicle_ys, label='Vehicle Path', color='red', linewidth=3, marker='o', markersize=5)
    ...
    if trajectory_data:
        ...
        plt.plot(traj_xs, traj_ys, label='Planned Trajectory', color='green', linestyle='--', linewidth=2)
    ...
    # Save the plot
    plot_path = os.path.join(plot_dir, f'comprehensive_trajectory_frame_{target_frame}.png')
...
def render_behavior_visualization(log_folder):
    ...
    try:
        ...
        plot_files = generate_behavior_plots(log_folder, behavior_file_path, target_frame)
        print(plot_files)
...
def load_log_data():
    ...
    for log_folder in sorted(os.listdir(LOG_DIR), reverse=True):
        ...
        logs.append({
            ...
            'launch_command': settings_data.get('log', {}).get('launch_command', 'Unknown'),
            'folder': log_path
        })
    ...
    try:
        ...
        if os.path.exists(settings_path):
            with open(settings_path, 'r') as settings_file:
                ...
                'launch_command': log_settings.get('log', {}).get('launch_command', 'Unknown'),
                })
    except Exception as e:
...
def open_folder(folder):
    # Determine the platform and try to open the folder
    if platform.system() == 'Linux':
        result = os.system(f'xdg-open "{folder}"')
    elif platform.system() == 'Windows':
        result = os.system(f'explorer "{folder}"')
    elif platform.system() == 'Darwin':  # macOS
        result = os.system(f'open "{folder}"')
    else:
