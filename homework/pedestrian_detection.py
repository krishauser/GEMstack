...
def refine_cluster(roi_points, center, eps=0.2, min_samples=10):
    ...
    if not valid_clusters:
        return roi_points
    best_cluster = min(
        valid_clusters, key=lambda c: np.linalg.norm(np.mean(c, axis=0) - center)
    )
    return best_cluster
...
class PedestrianDetector2D(Component):
    ...
    def initialize(self):
        ...
        self.vehicle_interface.subscribe_sensor(
            "front_camera", self.image_callback, cv2.Mat
        )
        self.vehicle_interface.subscribe_sensor(
            "lidar", self.lidar_callback, PointCloud2
        )
    ...
    def image_callback(self, image: cv2.Mat):
        ...
        boxes = (
            results[0].boxes.xywh.cpu().tolist()
        ...
        for i, box in enumerate(self.last_person_boxes):
            ...
            if roi_points.shape[0] < 20:
                ...
                refined_cluster = refine_cluster(
                    roi_points, intersection, eps=0.15, min_samples=10
                )
